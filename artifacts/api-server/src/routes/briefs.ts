...
router.post("/briefs/:id/send-to-pi", async (req, res): Promise<void> => {
  try {
    ...
    const bodyText = [
      "A new competitor intelligence briefing has been prepared for your review.",
      "",
      `This brief covers ${brief.alertCount} active alert${brief.alertCount !== 1 ? "s" : ""} detected from monitored competitor trials on ClinicalTrials.gov.`,
      "",
      `Open Intelligence Brief: ${brief.docUrl}`,
      "",
      "Please review and advise on any implications for our trial strategy. Reply to confirm receipt.",
      "",
